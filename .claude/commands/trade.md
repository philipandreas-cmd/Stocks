...
1. Pull state:
   ...
   Capture ask price P from quote.ap.
...
6. For BUYs, immediately place 10% trailing stop GTC:
   ...
   PDT fallback: fixed stop → queue tomorrow AM (same ladder as market-open).

7. Log to memory/TRADE-LOG.md with full thesis, entry, stop, target, R:R.

8. bash scripts/clickup.sh "<ticker, shares, fill price, one-line why>"
